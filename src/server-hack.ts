...
import { runScript, getServerList } from './utils';
...
export async function main(ns: NS): Promise<void> {
    ...
    function threadsToWeakenServer(hostname: string) {
        return Math.ceil((ns.getServerSecurityLevel(hostname) - ns.getServerMinSecurityLevel(hostname)) / 0.05);
    }
    ...
    function threadsToHackMoney(hostname: string, hackPercent: number) {
        ...
        if (hackAmount > 0) {
            return Math.ceil(ns.hackAnalyzeThreads(hostname, hackAmount) / ns.hackAnalyzeChance(hostname));
        }
        return 0;
    }
